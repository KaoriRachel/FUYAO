...
//    res = posix_memalign(&generate_payload, 4096, will_generate_payload_size);
    ...
    int ret = context->invoke_func_fn(context->caller_context,
                                      ...
                                      &fn_e_output, &fn_e_output_length, PassingMethod::DRC_OVER_IPC);
    auto end_time = std::stol(fn_e_output);

    if(ret != 0){
        return -1;
    }
    char fn_c_output[60];
    sprintf(fn_c_output, "Via DRC_OVER_IPC: interaction latency : %ld\n", end_time - start_time);
