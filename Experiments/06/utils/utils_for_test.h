...
#ifndef LUMINE_BENCHMARKS_UTILS_FOR_TEST_H
#define LUMINE_BENCHMARKS_UTILS_FOR_TEST_H
 ...
 * */
     ...
     *
     * example : exp01::utils_for_test::issue_http_request("POST", "localhost", 8080, "/function/exp01fnc", request_payload, buf, 1024)
     * **/
    ...
    int invoke_via_socket(std::string func_name, char *request_payload, int payload_length, char *response_payload) {
        // for response
        char buf[1024];
        int nrecved;

//    std::string gateway_addr, gateway_port;
        std::string gateway_addr = utils_for_test::GetEnvVariable("LUMINE_GATEWAY_ADDR", "192.168.1.129");
        std::string gateway_port = utils_for_test::GetEnvVariable("LUMINE_GATEWAY_PORT", "8084");
//
        auto request = utils_for_test::Socket(gateway_addr.c_str(), std::stoi(gateway_port));
//        auto request = utils_for_test::Socket("192.168.1.199", 8084);
        utils_for_test::HttpParser *http_parser = nullptr;
        ...
        http_parser = new utils_for_test::HttpParser(buf, nrecved);
        const char* response = http_parser->extract_payload();
        size_t response_len = strlen(response);
        strncpy(response_payload, response, response_len);
        request_payload[response_len] = '\0';
